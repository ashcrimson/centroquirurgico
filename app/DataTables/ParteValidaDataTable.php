...
class ParteValidaDataTable extends DataTable
{
    /**
    ...
    public function query(Parte $model)
    {
        ...
        if($user->hasRole(Role::PREOP_ANESTESISTA) ){
            $campoFecha = 'fecha_preop_anestesista_valida';
        }
        if($user->hasRole(Role::PREOP_EU) ){
            $campoFecha = 'fecha_preop_eu_valida';
        }

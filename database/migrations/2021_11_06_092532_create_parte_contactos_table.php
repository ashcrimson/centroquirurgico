...
class CreateParteContactosTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('parte_contactos', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('tipo_id')->nullable()->index('fk_parte_contactos_contacto_tipos1_idx');
            $table->unsignedBigInteger('parte_id')->index('fk_parte_contactos_partes1_idx');
            $table->unsignedBigInteger('parentesco_id')->index('fk_parte_contactos_parentescos1_idx');
            $table->string('nombre');
            $table->string('numero');
            $table->timestamps();
            $table->softDeletes();
